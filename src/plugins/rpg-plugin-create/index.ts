...
const plugin: Plugin = {
  ...
  async init(context: PluginContext) {
    ...
    context.program
      ...
      .action(async (templateName: string, projectName: string = '.', options: { force?: boolean }) => {
        try {
          ...
          if (context.packageManager) {
            ...
            try {
              ...
              if (Object.keys(allDependencies.dependencies).length > 0) {
                await projectPackageManager.installDependencies(
                  Object.entries(allDependencies.dependencies).map(([name, version]) => `${name}@${version}`)
                );
              }
              ...
              if (Object.keys(allDependencies.devDependencies).length > 0) {
                await projectPackageManager.installDependencies(
                  Object.entries(allDependencies.devDependencies).map(([name, version]) => `${name}@${version}`),
                  true // устанавливаем как devDependencies
                );
              }
