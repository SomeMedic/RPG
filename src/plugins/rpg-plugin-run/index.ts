...
const plugin: Plugin = {
  ...
  async init(context: PluginContext) {
    ...
    context.program
      ...
      .action(async (script: string | undefined, options: { list?: boolean }) => {
        try {
          ...
          if (!script || !scripts[script]) {
            ...
            const similarScripts = Object.keys(scripts).filter(name => 
              script ? (name.includes(script) || script.includes(name)) : false
            );
